...
CLIPTextModel = getattr(transformers, 'CLIPTextModel')
CLIPTokenizer = getattr(transformers, 'CLIPTokenizer')
CLIPModel = getattr(transformers, 'CLIPModel')
CLIPTextConfig = getattr(transformers, 'CLIPTextConfig')
...
def main(args):
    ...
    print('Stable Diffusion ckpt path:',args.ckpt)
    ...
    for gen_iter in range(args.n_iter):
        ...
        while args.interactive or prompt_index < len(prompt_list):
            ...
            for seed in seeds:
                ...
                return( process_batch([b1], highres_fix))
                if len(batch_data) > 0 and batch_data[-1].ext != b1.ext:
                    return process_batch(batch_data, highres_fix)
                    batch_data.clear()
...
class StitchDiffusionArgs:
    def __init__(
        ...
        self.diffusers_xformers = diffusers_xformers
...
repo_dir = "stitchdiffusion/kohya-trainer"
network_weight = "stitchdiffusion/load/lora.safetensors"  # @param {'type':'string'}
network_mul = 0.7
...
model = "stitchdiffusion/load/pretrained_model/stable-diffusion-2-1-base.safetensors"  # @param {twgetype: "string"}
vae = "stitchdiffusion/load/vae/stablediffusion.vae.pt"  # @param {type: "string"}
outdir = "stitchdiffusion/output"  # @param {type: "string"}
scale = 7
sampler = "ddim"
...
my_args = StitchDiffusionArgs(
    v2=config['v2'],
    v_parameterization=config['v_parameterization'],
    network_module=[config['network_module']],
    network_weights=[config['network_weight']],
    network_mul=[config['network_mul']],
    network_args=eval(config['network_args']) if config['network_args'] else None,
    ckpt=config['ckpt'],
    outdir=config['outdir'],
    xformers=config['xformers'],
    vae=config['vae'] if config['vae'] else None,
    fp16=config['fp16'],
    W=config['W'],
    H=config['H'],
    ...
    steps=config['steps'],
    max_embeddings_multiples=config['max_embeddings_multiples'],
    batch_size=config['batch_size'],
    images_per_prompt=config['images_per_prompt'],
    clip_skip=config['clip_skip'] if not config['v2'] else None,
    prompt=config['prompt']
)
negative = "lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry"

# class StitchDiffusionPipeline():
#     def __init__(self, args: StitchDiffusionArgs):
#         self.args = args
#     def __call__(self, prompt):
#         self.args.prompt = f"360-degree panoramic image, {prompt} --n {negative}"
...
#     def __init__(self, args: StitchDiffusionArgs):
#         self.args = args
#         self.vae, self.text_encoder, self.unet, self.scheduler = self._initialize_components()
    
#     def _initialize_components(self):
#         # vae, text_encoder, unet, scheduler 초기화 로직을 여기에 추가 (예: 모델 로드 등)
#         vae = AutoencoderKL()
#         text_encoder = CLIPTextModel()
#         unet = UNet2DConditionModel()
#         scheduler = DDIMScheduler()
#         return vae, text_encoder, unet, scheduler
...
# class StitchDiffusion():
#     def __init__(self, args: StitchDiffusionArgs):
#         self.pipe = StitchDiffusionPipeline(args)
...
#     sd.inference("360-degree panoramic image, basketball court on a mountain", "ye.png")


from models import register

STITCHDIFFUSION_ARGS = my_args

@register('StitchDiffusion')
class StitchDiffusion:
    ...
    def _initialize_components(self):
        ...
        if not os.path.isfile(self.args.ckpt):
            files = glob.glob(self.args.ckpt)
            if len(files) == 1:
                self.args.ckpt = files[0]

        use_stable_diffusion_format = os.path.isfile(self.args.ckpt)
        print("Stable Diffusion ckpt path:", self.args.ckpt)
        
        # Load model components from checkpoint
        if use_stable_diffusion_format:
            print("Loading Stable Diffusion checkpoint...")
            self.text_encoder, self.vae, self.unet = model_util.load_models_from_stable_diffusion_checkpoint(
                self.args.v2, self.args.ckpt
            )
        else:
            raise ValueError("Stable Diffusion checkpoint is not found or invalid.")

        # Load additional VAE if provided
        if self.args.vae is not None:
            self.vae = model_util.load_vae(self.args.vae, self.dtype)
            print("Additional VAE loaded")

        # Load tokenizer
        print("Loading tokenizer...")
        if use_stable_diffusion_format:
            self.tokenizer = train_util.load_tokenizer(self.args)
        ...
        # Setting the device and type for model components
        self.vae.to(self.dtype).to(self.device)
        self.text_encoder.to(self.dtype).to(self.device)
        self.unet.to(self.dtype).to(self.device)
        
        # Load LoRA networks if specified
        if self.args.network_module:
            for i, network_module in enumerate(self.args.network_module):
                imported_module = importlib.import_module(network_module)
                network_mul = 1.0 if self.args.network_mul is None or len(self.args.network_mul) <= i else self.args.network_mul[i]
                network_kwargs = {}

                if self.args.network_args and i < len(self.args.network_args):
                    network_args = self.args.network_args[i].split(";")
                    for net_arg in network_args:
                        key, value = net_arg.split("=")
                        network_kwargs[key] = value

                network, weights_sd = imported_module.create_network_from_weights(
                    network_mul, self.args.network_weights[i], self.vae, self.text_encoder, self.unet, for_inference=True, **network_kwargs
                )

                if network is None:
                    raise ValueError("Failed to load LoRA network.")

                network.apply_to(self.text_encoder, self.unet)
                info = network.load_state_dict(weights_sd, False)
                print(f"Weights loaded: {info}")

                network.to(self.dtype).to(self.device)
                self.networks.append(network)
                
            print("All LoRA networks loaded.")
